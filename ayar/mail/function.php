...
foreach($db->query("SELECT lisans.lisans_id, kullanici.kullanici_id,kullanici.isim,kullanici.soyisim,lisans.lisans_bitis,lisans.lisans_durum,program.program_isim, kullanici.eposta, DATEDIFF(lisans_bitis,NOW()) as 'kalan_gun' FROM lisans  INNER JOIN   kullanici on lisans.kullanici_id = kullanici.kullanici_id INNER JOIN program on program.program_id = lisans.program_id") as $row)

{
	...
	// kalan gün sayısı ile işlem yapıldığında sürekli alert veriyordu 
	// x .php de çağırdığım için tüm sayfalarda çalışıyor
	if ($db){
...
$mail->Body = "Sayın. $isim $soyisim .$lisans_bitis.' Tarihinde $program lisansınızın süresi dolacaktır.Lisansınızı yeniletmezseniz programı kullanamayacaksınız.
İyi günler."; // Mailin içeriği
if(!$mail->Send()){
